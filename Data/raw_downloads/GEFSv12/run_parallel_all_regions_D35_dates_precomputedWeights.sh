...
# I've only actually setup weights for CONUS right now
...
conus_weight=$maskDir/conus_weights_GEFSv12.nc
hgt_conus_weight=$maskDir/conus_weights_hgt_d35_GEFSv12.nc
...
for file in d35*.grib2; do
    ...
    if test -f $final_out_conus; then
        ...
        ncl_convert2nc $file -L
        #Now use cdo to convert to specific coordinates
        cdo -b f32 remap,$conus_grid,$hgt_conus_weight -sellevel,"${pressure_level}" "${file_name}".nc $final_out_conus
        rm "${file_name}".nc
    fi
...
for file in d35*grib2; do
    ...
    if test -f $final_out_conus; then
        ...
        ncl_convert2nc $file -L
        #Now use cdo to convert to specific coordinates
        cdo -b f32 remap,$conus_grid,$conus_weight "${file_name}".nc $final_out_conus
