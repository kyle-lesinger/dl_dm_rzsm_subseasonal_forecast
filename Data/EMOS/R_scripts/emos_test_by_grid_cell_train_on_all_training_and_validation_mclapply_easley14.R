...
emos_function_all_days_skip_bad_dates <- function(grid_number){
  ...
  train_subset  <- data %>% filter(vdate < '2018010100')
  ...
  #the training set
    ...
    # # #Testing with using a control
    # tempTestFit <- ensembleMOSnormal(data_setup_output$training_leadN, trainingDays = training_weeks,
    #                                dates = string_validation_days_test[date_],
    #                                control = controlMOSnormal(scoringRule=c('log'), optimRule = 'BFGS',
    #                                                         coefRule=c('none'), varRule=c('square'),maxIter = as.integer(100)))

    # #truncated normal
    # tempTestFit <- ensembleMOSnormal(data_setup_output$training_leadN, trainingDays = training_weeks,
    #                                dates = string_validation_days_test[date_],
    #                                control = controlMOStruncnormal(scoringRule=c('crps'), optimRule = 'BFGS',
    #                                                         coefRule=c('none'), varRule=c('square'),maxIter = as.integer(100)))
    ...
    # parValues <- pars(tempTestFit, data_setup_output$training_leadN,dates=string_validation_days_test[date_]) 
    parValues <- pars(tempTestFit, data_setup_output$training_leadN)#mean and standard deviation from forecast ensembles
    

  
    
    # observations_single <- single_grid_cell_test[date_,'RZSM.obs']
    # 
    # print(crpsValues)
    # print(parValues)
    # print(observations_single)
    # 
    # observations_all <- single_grid_cell_test['RZSM.obs']
    # check_values <- cbind(parValues,observations_all)
    
    #check output 
    # test=single_grid_cell_test[date_,c('RZSM.0','RZSM.1','RZSM.2','RZSM.3','RZSM.4','RZSM.5','RZSM.6','RZSM.7','RZSM.8','RZSM.9','RZSM.10')]
    # sum(test *tempTestFit$B)+tempTestFit$a
    
    #Can manually inspect the quantila
    # single_quantiles <- quantileForecast(tempTestFit,  data_setup_output$training_leadN, quantiles = c(0.05,.1,.2,.3,.4,.5,.6,.7,.8,.9,0.95), dates = string_validation_days_test[date_])
    # all_quantiles <- quantileForecast(tempTestFit,  data_setup_output$training_leadN, quantiles = c(0.05,.1,.2,.3,.4,.5,.6,.7,.8,.9,0.95), dates = string_validation_days_test)
    # cbind(all_quantiles,observations_all )
    
    # print(paste('CRPS_values:', crpsValues))
    # print(paste('parValues: ', parValues))
...
#week 1 emos
lead_day<<-as.double('14') #added by bash script
...
save.image(paste0('Rworkspace_emos',lead_day,'test_predictions_only_from_training_and_validation_dataset.Rdata'))

write.csv(final_output,paste0('emos_completed_lead',lead_day,'_test_predictions_only_from_training_and_validation_dataset.csv'),row.names = FALSE)
