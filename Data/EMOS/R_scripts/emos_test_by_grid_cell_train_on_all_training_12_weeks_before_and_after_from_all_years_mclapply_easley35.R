...
emos_function_all_days_skip_bad_dates <- function(grid_number){
  ...
  data_setup <- function(date_){
    ...
    return_12_weeks_before_and_after <- function(fcst_date){
    ...
    train_weeks <- return_12_weeks_before_and_after(fcst_date)
    ...
    for (i in 1:length(out_train_dates)){
      ...
      new_year_out_train <- return_12_weeks_before_and_after(new_year_out_train)
      new_year_out_train <- str_sort(new_year_out_train)
      #Now we know what dates are at least 12 weeks before and after, now subset the out_train_dates
...
save.image(paste0('Rworkspace_emos',lead_day,'test_predictions_only_from_training_dataset_12_weeks_before_and_after.Rdata'))
write.csv(final_output,paste0('emos_completed_lead',lead_day,'_test_predictions_only_from_training_dataset_12_weeks_before_and_after.csv'),row.names = FALSE)
