...
def load_all_data_EX0(lead, num_lags_obs_RZSM, include_lags_obs_pwat_spfh_tmax, include_reforecast_or_not, observation_lag_list_not_RZSM, lag_integer_list, input_directory,training_size_shape,validation_testing_size_shape,RZSM_or_Tmax_or_both,addtl_experiment,experiment_test,ref_source,final_testing_year):
...
def load_all_data_EX1_EX2_EX3_EX4(lead, num_lags_obs_RZSM, include_lags_obs_pwat_spfh_tmax, include_reforecast_or_not, observation_lag_list_not_RZSM, lag_integer_list, input_directory,training_size_shape,validation_testing_size_shape,RZSM_or_Tmax_or_both,region_name):
    ...
    for idx,lag in enumerate(lag_integer_list):
        ...
        with open(f'{input_directory}/OBS_RZSM_GLEAM_training.pickle', 'rb') as handle:
            training_input[:,:,:,idx] = pickle.load(handle)[f'Lag{lag}'].astype(np.float32)
            #print(f'\n\n Idx {idx} RZSM obs : {training_input[:,:,:,idx]}')
            
        with open(f'{input_directory}/OBS_RZSM_GLEAM_validation.pickle', 'rb') as handle:
            validation_input[:,:,:,idx] = pickle.load(handle)[f'Lag{lag}'].astype(np.float32)
            
        with open(f'{input_directory}/OBS_RZSM_GLEAM_testing.pickle', 'rb') as handle:
    ...
    for lag in observation_lag_list_not_RZSM:
        ...
        #Observations pwat
        with open(f'{input_directory}/OBS_pwat_ERA5_training.pickle', 'rb') as handle:
            training_input[:,:,:,idx] = pickle.load(handle)[f'Lag{lag}'].astype(np.float32)
            #print(f'\n\n Idx {idx} pwat obs : {training_input[:,:,:,idx]}')
        with open(f'{input_directory}/OBS_pwat_ERA5_validation.pickle', 'rb') as handle:
            validation_input[:,:,:,idx] = pickle.load(handle)[f'Lag{lag}'].astype(np.float32)
        with open(f'{input_directory}/OBS_pwat_ERA5_testing.pickle', 'rb') as handle:
            testing_input[:,:,:,idx] = pickle.load(handle)[f'Lag{lag}'].astype(np.float32)   
    ...
    for lag in observation_lag_list_not_RZSM:
        ...
        idx+=1
        #Observations spfh
        with open(f'{input_directory}/OBS_spfh_ERA5_training.pickle', 'rb') as handle:
            training_input[:,:,:,idx] = pickle.load(handle)[f'Lag{lag}'].astype(np.float32)
            #print(f'\n\n Idx {idx} spfh obs : {training_input[:,:,:,idx]}')
        with open(f'{input_directory}/OBS_spfh_ERA5_validation.pickle', 'rb') as handle:
            validation_input[:,:,:,idx] = pickle.load(handle)[f'Lag{lag}'].astype(np.float32)
        with open(f'{input_directory}/OBS_spfh_ERA5_testing.pickle', 'rb') as handle:
            testing_input[:,:,:,idx] = pickle.load(handle)[f'Lag{lag}'].astype(np.float32)   
    ...
    for lag in observation_lag_list_not_RZSM:
        ...
        idx+=1
        #Observations tmax
        with open(f'{input_directory}/OBS_tmax_ERA5_training.pickle', 'rb') as handle:
            training_input[:,:,:,idx] = pickle.load(handle)[f'Lag{lag}'].astype(np.float32)
            #print(f'\n\n Idx {idx} tmax obs : {training_input[:,:,:,idx]}')
        with open(f'{input_directory}/OBS_tmax_ERA5_validation.pickle', 'rb') as handle:
            validation_input[:,:,:,idx] = pickle.load(handle)[f'Lag{lag}'].astype(np.float32)
        with open(f'{input_directory}/OBS_tmax_ERA5_testing.pickle', 'rb') as handle:
            testing_input[:,:,:,idx] = pickle.load(handle)[f'Lag{lag}'].astype(np.float32)  
    ...
    for lag in observation_lag_list_not_RZSM:
        ...
        idx+=1
        #Observations tmax
        with open(f'{input_directory}/OBS_diff_temp_ERA5_training.pickle', 'rb') as handle:
            training_input[:,:,:,idx] = pickle.load(handle)[f'Lag{lag}'].astype(np.float32)
            #print(f'\n\n Idx {idx} diff temp obs : {training_input[:,:,:,idx]}')
        with open(f'{input_directory}/OBS_diff_temp_ERA5_validation.pickle', 'rb') as handle:
            validation_input[:,:,:,idx] = pickle.load(handle)[f'Lag{lag}'].astype(np.float32)
        with open(f'{input_directory}/OBS_diff_temp_ERA5_testing.pickle', 'rb') as handle:
            testing_input[:,:,:,idx] = pickle.load(handle)[f'Lag{lag}'].astype(np.float32)  
    ...
    for lag in observation_lag_list_not_RZSM:
        ...
        idx+=1
        #Observations tmax
        with open(f'{input_directory}/OBS_geopotential_z200_ERA5_training.pickle', 'rb') as handle:
            training_input[:,:,:,idx] = pickle.load(handle)[f'Lag{lag}'].astype(np.float32)
            #print(f'\n\n Idx {idx} z200 obs : {training_input[:,:,:,idx]}')
        with open(f'{input_directory}/OBS_geopotential_z200_ERA5_validation.pickle', 'rb') as handle:
            validation_input[:,:,:,idx] = pickle.load(handle)[f'Lag{lag}'].astype(np.float32)
        with open(f'{input_directory}/OBS_geopotential_z200_ERA5_testing.pickle', 'rb') as handle:
            testing_input[:,:,:,idx] = pickle.load(handle)[f'Lag{lag}'].astype(np.float32)  
...
def load_all_data_EX5_EX6_EX7_EX8(lead, num_lags_obs_RZSM, include_lags_obs_pwat_spfh_tmax, include_reforecast_or_not, observation_lag_list_not_RZSM, lag_integer_list, input_directory,training_size_shape,validation_testing_size_shape,RZSM_or_Tmax_or_both,region_name):
    ...
    for i in lead_list:
        ...
        idx+=1
        with open(f'{input_directory}/REFORECAST_RZSM_GEFSv12_training.pickle', 'rb') as handle:
            training_input[:,:,:,idx] = pickle.load(handle)[f'Lead{i}'].astype(np.float32)
        with open(f'{input_directory}/REFORECAST_RZSM_GEFSv12_validation.pickle', 'rb') as handle:
            validation_input[:,:,:,idx] = pickle.load(handle)[f'Lead{i}'].astype(np.float32)
        with open(f'{input_directory}/REFORECAST_RZSM_GEFSv12_testing.pickle', 'rb') as handle:
    ...
    if RZSM_or_Tmax_or_both == 'both':
        ...
        for i in (lead_list):
            ...
            idx+=1
            with open(f'{input_directory}/REFORECAST_tmax_GEFSv12_training.pickle', 'rb') as handle:
                training_input[:,:,:,idx] = pickle.load(handle)[f'Lead{i}'].astype(np.float32)
            with open(f'{input_directory}/REFORECAST_tmax_GEFSv12_validation.pickle', 'rb') as handle:
                validation_input[:,:,:,idx] = pickle.load(handle)[f'Lead{i}'].astype(np.float32)
            with open(f'{input_directory}/REFORECAST_tmax_GEFSv12_testing.pickle', 'rb') as handle:
                testing_input[:,:,:,idx] = pickle.load(handle)[f'Lead{i}'].astype(np.float32)
...
def load_all_data_EX9_EX10_EX11_EX12(lead, num_lags_obs_RZSM, include_lags_obs_pwat_spfh_tmax, include_reforecast_or_not, observation_lag_list_not_RZSM, lag_integer_list, input_directory,training_size_shape,validation_testing_size_shape,RZSM_or_Tmax_or_both, experiment_test,region_name):
    ...
    if RZSM_or_Tmax_or_both == 'RZSM' and experiment_test == 0:
        lead_add = 1
    elif RZSM_or_Tmax_or_both == 'both':
        lead_add = 2
    elif RZSM_or_Tmax_or_both == 'RZSM' and experiment_test == 2:
    ...
    # (tmax, diff_temp, z200, pwat, spfh)
    if experiment_test >= 1:
        channel_list.append(f'RZSM_tmax_lead{i}')
        idx+=1
        #reforecast tmax
        with open(f'{input_directory}/REFORECAST_RZSM_GEFSv12_training.pickle', 'rb') as handle:
            training_input[:,:,:,idx] = pickle.load(handle)[f'Lead{i}'].astype(np.float32)
